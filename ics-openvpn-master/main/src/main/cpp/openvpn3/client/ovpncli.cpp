...
namespace openvpn {
    namespace ClientAPI {
            ...
            virtual RemoteList::Item::Ptr get() override {
                if (parent) {
                    ...
                    if (ro.error.empty()) {
                        ...
                        ri->transport_protocol = Protocol::parse(ro.proto, Protocol::CLIENT_SUFFIX,
                                                                 title.c_str());
                    } else
        ...
        namespace Private {
            ...
            public:
                ...
                void setup_async_stop_scopes() {
                    stop_scope_local.reset(
                            new AsioStopScope(*io_context(), async_stop_local(), [this]() {
                                OPENVPN_ASYNC_HANDLER;
                                session->graceful_stop();
                            }));

                    stop_scope_global.reset(
                            new AsioStopScope(*io_context(), async_stop_global(), [this]() {
                                OPENVPN_ASYNC_HANDLER;
        ...
        OPENVPN_CLIENT_EXPORT void
        OpenVPNClientHelper::parse_config(const Config &config, EvalConfig &eval,
                                          OptionList &options) {
            try {
                ...
                const ParseClientConfig cc = ParseClientConfig::parse(config.content, &kvl,
                                                                      options);
                ...
                eval.remoteHost = config.serverOverride.empty() ? cc.firstRemoteListItem().host
                ...
                for (ParseClientConfig::ServerList::const_iterator i = cc.serverList().begin();
                     i != cc.serverList().end(); ++i) {
                    ServerEntry se;
                    se.server = i->server;
                ...
                eval.error = true;
                eval.message = Unicode::utf8_printable<std::string>(
                        std::string("ERR_PROFILE_GENERIC: ") + e.what(), 256);
            }
        }

        OPENVPN_CLIENT_EXPORT void
        OpenVPNClient::parse_extras(const Config &config, EvalConfig &eval) {
            try {
                ...
                if (!config.allowUnusedAddrFamilies.empty())
                    state->allowUnusedAddrFamilies = TriStateSetting::parse(
        ...
        OPENVPN_CLIENT_EXPORT MergeConfig OpenVPNClientHelper::merge_config(const std::string &path,
            ...
            ProfileMerge pm(path, "ovpn", "", follow_references ? ProfileMerge::FOLLOW_PARTIAL
        ...
        OPENVPN_CLIENT_EXPORT MergeConfig
        OpenVPNClientHelper::merge_config_string(const std::string &config_content) {
        ...
        OPENVPN_CLIENT_EXPORT MergeConfig
        OpenVPNClientHelper::build_merge_config(const ProfileMerge &pm) {
            MergeConfig ret;
        ...
        OPENVPN_CLIENT_EXPORT bool
        OpenVPNClient::socket_protect(int socket, std::string remote, bool ipv6) {
            return true;
        }

        OPENVPN_CLIENT_EXPORT bool
        OpenVPNClientHelper::parse_dynamic_challenge(const std::string &cookie,
                                                     DynamicChallenge &dc) {
            try {
        ...
        OPENVPN_CLIENT_EXPORT void
        OpenVPNClient::process_epki_cert_chain(const ExternalPKICertRequest &req) {
...
#error ovpn3 core logging object only supports thread-local scope
        ...
        OPENVPN_CLIENT_EXPORT void OpenVPNClient::do_connect_async() {
            ...
            if (status.error) {
                ClientEvent::Base::Ptr ev = new ClientEvent::ClientSetup(status.status,
                                                                         status.message);
        ...
        OPENVPN_CLIENT_EXPORT void
        OpenVPNClient::connect_setup(Status &status, bool &session_started) {
            ...
            if (state->clock_tick_ms) {
                state->clock_tick.reset(
                        new MyClockTick(*state->io_context(), this, state->clock_tick_ms));
        ...
        OPENVPN_CLIENT_EXPORT void
        OpenVPNClient::external_pki_error(const ExternalPKIRequestBase &req,
        ...
        OPENVPN_CLIENT_EXPORT bool
        OpenVPNClient::sign(const std::string &data, std::string &sig, const std::string &algorithm,
                ...
                if (stats) {
                    ...
                    // calculate time since last packet received
                    {
                        ...
                        if (lpr.defined()) {
                            ...
                            if (delta <=
                                60 * 60 * 24 * 1024) // only define for time periods <= 1 day
        ...
        OPENVPN_CLIENT_EXPORT std::string OpenVPNClientHelper::cryptoSelfTest() {
